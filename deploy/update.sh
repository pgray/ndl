...
if [ "$PREV_IMAGES" = "$NEW_IMAGES" ]; then
    log "No new images, skipping restart"
    exit 0
fi

log "New images detected, restarting services..."
...
log "WARNING: Health check failed after ${HEALTH_TIMEOUT}s (service may still be starting)"
exit 0
